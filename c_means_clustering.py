...
def inverse_sum_PPV_dist(node, neighbors, cluster):
    std_dist = 1000
...
def edge_ratio(node, neighbors, cluster):
...
def node_c_distances(node, neighbors, clusters, distance_measure_f): # add weight formula argument:
    node_distances = dict()
...
def node_iteration(node, neighbors, clusters, m, optimize, distance_measure_f):
    node_distances = node_c_distances(node, neighbors, clusters, distance_measure_f) # returns dictionary with distance to each cluster
...
def update_clusters_z(memberships_dict, clusters, alpha=0.05):
    for (node, memberships) in memberships_dict.items():
        memberships_distr = memberships.values()
        memberships_mean = mean(memberships_distr)
        memberships_sd = stdev(memberships_distr)
        z_thresh = (2.33 * memberships_sd) + memberships_mean # 1.65 for 0.05, 2.33 for 0.01

        node_clusters = [cluster_id for (cluster_id, membership) in memberships.items() if membership > z_thresh]
...
def network_c_means(graph, clusters, m, n_iter, optimize=False, percentile=0.95, t = 0.5, distance_measure='dist_sum', path_lengths=None, cores=6):
    ...
    distance_measure_f = distance_measures[distance_measure]
    print(distance_measure_f)
    for i in range(1,n_iter+1):
        print(f'Iteration {i} of {n_iter}')

        results = parallel(delayed(node_iteration)(node, connections_dict[node], clusters, m, optimize, distance_measure_f) for node in tqdm(nodes))
        ...
        # clusters = update_clusters_z(memberships_dict, clusters)
